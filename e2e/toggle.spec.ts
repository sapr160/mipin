...
test.describe("language toggle", () => {
  test("switches all visible text and persists across reloads", async ({
    ...
    await page.goto("/");
    await expect(page.getByRole("heading", { name: /Intercambia pines/ })).toBeVisible();

    await page.getByRole("button", { name: "EN" }).click();

    await expect(page.getByRole("heading", { name: /Swap pins/ })).toBeVisible();
  ...
  test("an explicit choice beats Accept-Language on later visits", async ({
    ...
    await later.goto("/");
    await expect(later.getByRole("heading", { name: /Swap pins/ })).toBeVisible();
